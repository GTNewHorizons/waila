...
public class Waila {
    ...
    public void initialize(final FMLInitializationEvent event) {
        try {
            final Field eBus = FMLModContainer.class.getDeclaredField("eventBus");
            eBus.setAccessible(true);
            final EventBus FMLbus = (EventBus) eBus.get(FMLCommonHandler.instance().findContainerFor(this));
    ...
    public void processIMC(final FMLInterModComms.IMCEvent event) {
        for (final IMCMessage imcMessage : event.getMessages()) {
            ...
            if (imcMessage.key.equalsIgnoreCase("addconfig")) {
                final String[] params = imcMessage.getStringValue().split("\\$\\$");
                if (params.length != 3) {
