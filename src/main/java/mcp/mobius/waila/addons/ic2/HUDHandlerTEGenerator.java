...
public class HUDHandlerTEGenerator implements IWailaDataProvider {
    ...
    public List<String> getWailaBody(final ItemStack itemStack, List<String> currenttip, final IWailaDataAccessor accessor, final IWailaConfigHandler config) {
        try {
            final double storage = accessor.getNBTData().getDouble("storage");
            final int production = accessor.getNBTData().getInteger("production");
            final long maxStorage = accessor.getNBTData().getLong("maxStorage");

            final String storedStr = LangUtil.translateG("hud.msg.stored");
