...
public class HUDHandlerTesseract implements IWailaDataProvider {
    ...
    public List<String> getWailaBody(ItemStack itemStack, List<String> currenttip, IWailaDataAccessor accessor,
        ...
        if (config.getConfig("thermalexpansion.tesssendrecv")) {
            ...
            if (!send.equals(String.format("%s : ", LangUtil.translateG("hud.msg.send")))) currenttip.add(send);

            if (!send.equals(String.format("%s : ", LangUtil.translateG("hud.msg.recv")))) currenttip.add(recv);
        }

        if (config.getConfig("thermalexpansion.tessfreq")) currenttip.add(
                String.format(
                        "%s : %d",
                        LangUtil.translateG("hud.msg.frequency"),
