...
public class HUDHandlerVanilla implements IWailaDataProvider {
    ...
    public ItemStack getWailaStack(final IWailaDataAccessor accessor, final IWailaConfigHandler config) {
        ...
        if (block == silverfish && config.getConfig("vanilla.silverfish")) {
            final int metadata = accessor.getMetadata();
            switch (metadata) {
                case 0:
        ...
        if (block == doubleplant && (accessor.getMetadata() & 8) != 0) {
            final int x = accessor.getPosition().getX();
            int y = accessor.getPosition().getY() - 1;
            final int z = accessor.getPosition().getZ();
            if (accessor.getWorld().getBlockState(new BlockPos(x, y, z)) != Blocks.DOUBLE_PLANT)
                y += 1; // We only have the top half of a double plant!
            final EnumPlantType variant = accessor.getWorld().getBlockState(new BlockPos(x, y, z)).getValue(BlockDoublePlant.VARIANT);
            switch (variant) {
                case FERN:
        ...
        if (block == flowerpot) {
            final int x = accessor.getPosition().getX();
            final int y = accessor.getPosition().getY();
            final int z = accessor.getPosition().getZ();
            ...
            final EnumFlowerType variant = accessor.getWorld().getBlockState(new BlockPos(x, y, z)).getValue(BlockFlowerPot.CONTENTS);
            switch (variant) {
                case ACACIA_SAPLING:
    ...
    public List<String> getWailaHead(final ItemStack itemStack, final List<String> currenttip, final IWailaDataAccessor accessor, final IWailaConfigHandler config) {
        ...
        if (block == mobSpawner && accessor.getTileEntity() instanceof TileEntityMobSpawner && config.getConfig("vanilla.spawntype")) {
            //TODO
            final String name = currenttip.get(0);
            final String mobname = ((TileEntityMobSpawner) accessor.getTileEntity()).getSpawnerBaseLogic().getCachedEntity().getName();//(accessor.getWorld()).getName();
            currenttip.set(0, String.format("%s (%s)", name, mobname));
        }

        if (block == redstone) {
            final String name = currenttip.get(0).replaceFirst(String.format(" %s", accessor.getMetadata()), "");
    ...
    public List<String> getWailaBody(final ItemStack itemStack, final List<String> currenttip, final IWailaDataAccessor accessor, final IWailaConfigHandler config) {
        final Block block = accessor.getBlock();
        /* Crops */
        final boolean iscrop = crops.getClass().isInstance(block);    //Done to cover all inheriting mods
        if (config.getConfig("general.showcrop"))
            if (iscrop || block == melonStem || block == pumpkinStem || block == carrot || block == potato) {
                final float growthValue = (accessor.getMetadata() / 7.0F) * 100.0F;
        ...
        if (block == cocoa && config.getConfig("general.showcrop")) {
            ...
            final float growthValue = ((accessor.getMetadata() >> 2) / 2.0F) * 100.0F;
            if (growthValue < 100.0)
                currenttip.add(String.format("%s : %.0f %%", LangUtil.translateG("hud.msg.growth"), growthValue));
            else
                currenttip.add(String.format("%s : %s", LangUtil.translateG("hud.msg.growth"), LangUtil.translateG("hud.msg.mature")));
        ...
        if (block == netherwart && config.getConfig("general.showcrop")) {
            final float growthValue = (accessor.getMetadata() / 3.0F) * 100.0F;
            if (growthValue < 100.0)
                currenttip.add(String.format("%s : %.0f %%", LangUtil.translateG("hud.msg.growth"), growthValue));
            else
                currenttip.add(String.format("%s : %s", LangUtil.translateG("hud.msg.growth"), LangUtil.translateG("hud.msg.mature")));
        ...
        if (config.getConfig("vanilla.leverstate"))
            if (block == lever) {
                final String redstoneOn = (accessor.getMetadata() & 8) == 0 ? LangUtil.translateG("hud.msg.off") : LangUtil.translateG("hud.msg.on");
        ...
        if (config.getConfig("vanilla.repeater"))
            if ((block == repeaterIdle) || (block == repeaterActv)) {
                final int tick = (accessor.getMetadata() >> 2) + 1;
                if (tick == 1)
        ...
        if (config.getConfig("vanilla.comparator"))
            if ((block == comparatorIdl) || (block == comparatorAct)) {
                final String mode = ((accessor.getMetadata() >> 2) & 1) == 0 ? LangUtil.translateG("hud.msg.comparator") : LangUtil.translateG("hud.msg.substractor");
        ...
        if (config.getConfig("vanilla.jukebox"))
            if (block == jukebox) {
                ...
                if (tag.hasKey("RecordItem", 10)) {
                    final Item record = new ItemStack(tag.getCompoundTag("RecordItem")).getItem();
    ...
    public static void register() {
        ...
        final IWailaDataProvider provider = new HUDHandlerVanilla();
        ...
        for (final Block b : removemetadata) {
