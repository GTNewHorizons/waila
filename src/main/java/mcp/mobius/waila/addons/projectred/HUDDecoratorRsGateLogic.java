...
public class HUDDecoratorRsGateLogic implements IWailaFMPDecorator {
    ...
    @Override
    public void decorateBlock(final ItemStack itemStack, final IWailaFMPAccessor accessor, final IWailaConfigHandler config) {
        ...
        final EnumFacing vOrient = EnumFacing.values()[(orient - (orient & 0x3)) >> 2];
        ...
        final String[] IOStr = new String[4];
        if (shape != 0) {
            int altIndex = -1;
        ...
        final String[] IOStrRot = new String[4];
        ...
        final double x = accessor.getRenderingPosition().xCoord;
        final double y = accessor.getRenderingPosition().yCoord;
        final double z = accessor.getRenderingPosition().zCoord;
