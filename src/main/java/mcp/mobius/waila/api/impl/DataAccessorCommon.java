...
public class DataAccessorCommon implements IWailaCommonAccessor, IWailaDataAccessor, IWailaEntityAccessor {
    ...
    public void set(final World _world, final EntityPlayer _player, final RayTraceResult _mop) {
        this.set(_world, _player, _mop, null, 0.0);
    }

    public void set(final World _world, final EntityPlayer _player, final RayTraceResult _mop, final Entity viewEntity, final double partialTicks) {
        this.world = _world;
        this.player = _player;
        this.mop = _mop;
        ...
        if (viewEntity != null) {
            final double px = viewEntity.lastTickPosX + (viewEntity.posX - viewEntity.lastTickPosX) * partialTicks;
            final double py = viewEntity.lastTickPosY + (viewEntity.posY - viewEntity.lastTickPosY) * partialTicks;
            final double pz = viewEntity.lastTickPosZ + (viewEntity.posZ - viewEntity.lastTickPosZ) * partialTicks;
    ...
    private boolean isTagCorrectTileEntity(final NBTTagCompound tag) {
        if (tag == null || !tag.hasKey("WailaX")) {
            this.timeLastUpdate = System.currentTimeMillis() - 250;
            return false;
        }

        final int x = tag.getInteger("WailaX");
        final int y = tag.getInteger("WailaY");
        final int z = tag.getInteger("WailaZ");
    ...
    private boolean isTagCorrectEntity(final NBTTagCompound tag) {
        if (tag == null || !tag.hasKey("WailaEntityID")) {
            this.timeLastUpdate = System.currentTimeMillis() - 250;
            return false;
        }

        final int id = tag.getInteger("WailaEntityID");
    ...
    public boolean isTimeElapsed(final long time) {
