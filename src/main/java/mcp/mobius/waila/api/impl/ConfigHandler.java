...
public class ConfigHandler implements IWailaConfigHandler {
    ...
    public HashMap<String, String> getConfigKeys(String modName) {
        if (this.modules.containsKey(modName)) return this.modules.get(modName).options;
        else return null;
    }
    ...
    public void addConfig(String modName, String key, String name, boolean defvalue) {
        ...
        if (!this.modules.containsKey(modName)) this.modules.put(modName, new ConfigModule(modName));
    ...
    public boolean getConfig(String key, boolean defvalue) {
        if (this.serverconfigs.contains(key) && !Waila.instance.serverPresent) return false;
    ...
    public void loadDefaultConfig(FMLPreInitializationEvent event) {
        ...
        config.get(Configuration.CATEGORY_GENERAL, Constants.CFG_WAILA_FORCE_LEGACY_MODE, false);
        ...
        OverlayConfig.gradient1 = config.get(Configuration.CATEGORY_GENERAL, Constants.CFG_WAILA_GRADIENT1, 0x5000ff)
                .getInt();
        OverlayConfig.gradient2 = config.get(Configuration.CATEGORY_GENERAL, Constants.CFG_WAILA_GRADIENT2, 0x28007f)
                .getInt();
        ...
        HUDHandlerEntities.nhearts = config.get(Configuration.CATEGORY_GENERAL, Constants.CFG_WAILA_NHEARTS, 20)
                .getInt();
        HUDHandlerEntities.maxhpfortext = config.get(Configuration.CATEGORY_GENERAL, Constants.CFG_WAILA_MAXHP, 40)
                .getInt();
