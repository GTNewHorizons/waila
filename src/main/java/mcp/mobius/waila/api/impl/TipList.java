...
public class TipList<E, T> extends ArrayList<E> implements ITaggedList<E, T> {
    ...
    public void removeEntries(T tag) {
        for (E e : this.getEntries(tag)) this.remove(e);
    }
    ...
    protected void removeRange(int fromIndex, int toIndex) {
        for (int i = fromIndex; i < toIndex; i++) tags.remove(this.get(i));
    ...
    public boolean retainAll(Collection<?> c) {
        for (E e : tags.keySet()) if (!c.contains(e)) tags.remove(e);

        return super.retainAll(c);
    }

    /*
     * public boolean add(E e){ this.setInitialTag(e); return super.add(e); } public void add(int index, E element){
     * this.setInitialTag(element); super.add(index, element); } public boolean addAll(Collection<? extends E> c){ if (c
     * instanceof TipList){ for (E e : c) tags.put(e, ((TipList)c).getTags(e)); } else { for (E e : c)
     * this.setInitialTag(e); } return super.addAll(c); } public boolean addAll(int index, Collection<? extends E> c){
     * if (c instanceof TipList){ for (E e : c) tags.put(e, ((TipList)c).getTags(e)); } else { for (E e : c)
     * this.setInitialTag(e); } return super.addAll(index, c); } public void clear(){ tags.clear(); super.clear(); }
     * public Object clone(){ return super.clone(); } private void setInitialTag(E key){ tags.put(key, new ArrayList());
