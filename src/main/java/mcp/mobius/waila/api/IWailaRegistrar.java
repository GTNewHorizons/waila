 ...
 * <p>
 * If not specified otherwise, all the registration methods taking a class can take classes as well as interfaces. Waila
 * will do a lookup using instanceof on the registered classes, meaning that if all your targets inherit one interface,
 * you only need to specify it to cover the whole hierarchy.<br>
...
public interface IWailaRegistrar {
     ...
     * Accept * as a ending wildcard registerNBTKey("bob.*", MyBlock.class) registerNBTKey("data.life", MyEntity.class)
     * registerNBTKey("*", MyTileEntity.class) will reproduce the full tag syncing from 1.4.5 This registration method
