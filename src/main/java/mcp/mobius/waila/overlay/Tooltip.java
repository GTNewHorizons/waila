...
public class Tooltip {
    ...
    private class Renderable {
        ...
        public Renderable(final IWailaTooltipRenderer renderer, final Point pos, final String[] params) {
            this.renderer = renderer;
            this.pos = pos;
            this.params = params;
        }

        public Renderable(final IWailaTooltipRenderer renderer, final Point pos) {
        ...
        public Dimension getSize(final IWailaCommonAccessor accessor) {
        ...
        public void draw(final IWailaCommonAccessor accessor, final int x, final int y) {
            GL11.glPushMatrix();
    ...
    public Tooltip(final List<String> textData, final ItemStack stack) {
        this(textData, true);
        this.stack = stack;
    }

    public Tooltip(final List<String> textData, final boolean hasIcon) {

        columnsWidth.add(0);        // Small init of the arrays to have at least one element
        columnsPos.add(0);

        for (final String s : textData) {

            final ArrayList<String> line = new ArrayList<String>(Arrays.asList(patternTab.split(s)));
            final ArrayList<Integer> size = new ArrayList<Integer>();
            for (final String ss : line)
    ...
    private void computeRenderables() {
        ...
        for (int i = 0; i < lines.size(); i++) {                // We check all the lines, one by one
            ...
            for (int c = 0; c < lines.get(i).size(); c++) {    // We check all the columns for this line
                ...
                final String currentLine = lines.get(i).get(c);
                final Matcher lineMatcher = patternLineSplit.matcher(currentLine);

                while (lineMatcher.find()) {
                    final String cs = lineMatcher.group();
                    Renderable renderable = null;
                    final Matcher renderMatcher = patternRender.matcher(cs);    //We keep a matcher here to be able to check if we have a Renderer. Might be better to do a startWith + full matcher init after the check
                    final Matcher iconMatcher = patternIcon.matcher(cs);

                    if (renderMatcher.find()) {
                        final String renderName = renderMatcher.group("name");

                        final IWailaTooltipRenderer renderer = ModuleRegistrar.instance().getTooltipRenderer(renderName);
                        if (renderer != null) {
    ...
    private int getRenderableTotalHeight() {
        int result = 0;
        for (final Renderable r : this.elements)
            result = Math.max(r.getPos().y + r.getSize(accessor).height + 2, result);
        return result;
    }

    private void computePositionAndSize(final boolean hasIcon) {
        ...
        final int paddingW = hasIcon ? 29 : 13;
        final int paddingH = hasIcon ? 24 : 0;
        ...
        final Dimension size = DisplayUtil.displaySize();
    ...
    public void draw2nd() {
        for (final Renderable r : this.elements2nd)
            r.draw(accessor, x + offsetX, y + ty);
    }
}
