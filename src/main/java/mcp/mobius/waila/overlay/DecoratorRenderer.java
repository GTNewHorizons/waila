...
public class DecoratorRenderer {
    ...
    public void onRenderWorldLast(final RenderWorldLastEvent event) {
        ...
        if (ModuleRegistrar.instance().hasBlockDecorator(block)) {
            for (final List<IWailaBlockDecorator> decoratorsList : ModuleRegistrar.instance().getBlockDecorators(block).values()) {
                for (final IWailaBlockDecorator decorator : decoratorsList)
                    try {
                        GL11.glPushMatrix();
