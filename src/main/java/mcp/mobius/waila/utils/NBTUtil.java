...
public class NBTUtil {
    ...
    public static NBTBase getTag(final String key, final NBTTagCompound tag) {
    ...
    public static NBTTagCompound setTag(final String key, final NBTTagCompound targetTag, final NBTBase addedTag) {
        final String[] path = key.split("\\.");
    ...
    public static NBTTagCompound createTag(final NBTTagCompound inTag, final HashSet<String> keys) {
        if (keys.contains("*")) return inTag;

        NBTTagCompound outTag = new NBTTagCompound();

        for (final String key : keys) {
            final NBTBase tagToAdd = getTag(key, inTag);
            //System.out.printf("%s\n", tagToAdd);
            if (tagToAdd != null)
...
/*    public static NBTTagCompound readNBTTagCompound(DataInputStream par0DataInputStream) throws IOException
    ...
    public static int getNBTInteger(final NBTTagCompound tag, final String keyname) {
        final NBTBase subtag = tag.getTag(keyname);
