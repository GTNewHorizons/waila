...
public class ScreenHUDConfig extends ScreenBase {
    ...
    private class EventCanvas extends LayoutBase {
        ...
        public EventCanvas(final IWidget parent, final GuiScreen prevScreen) {
            super(parent);
			...
			*/
            ...
            final double picX = ConfigHandler.instance().getConfig(Configuration.CATEGORY_GENERAL, Constants.CFG_WAILA_POSX, 0) / 100.0;
            final double picY = ConfigHandler.instance().getConfig(Configuration.CATEGORY_GENERAL, Constants.CFG_WAILA_POSY, 0) / 100.0;
            final float picAlpha = ConfigHandler.instance().getConfig(Configuration.CATEGORY_GENERAL, Constants.CFG_WAILA_ALPHA, 0) / 100.0f;
            scale = ConfigHandler.instance().getConfig(Configuration.CATEGORY_GENERAL, Constants.CFG_WAILA_SCALE, 0) / 100.0f;

            final int picSX = (int) (180 / this.rez.getScaleFactor() * scale);
            final int picSY = (int) (62 / this.rez.getScaleFactor() * scale);
            ...
            final IWidget layoutX = this.addWidget("LayoutX", new LayoutBase(this));
            ...
            final IWidget layoutY = this.addWidget("LayoutY", new LayoutBase(this));
            ...
            final IWidget layoutAlpha = this.addWidget("LayoutAlpha", new LayoutBase(this));
            ...
            final IWidget layoutScale = this.addWidget("LayoutScale", new LayoutBase(this));
        ...
        @Override
        public void onWidgetEvent(final IWidget srcwidget, final Signal signal, final Object... params) {
            ...
            if (signal == Signal.CLICKED) {
                ...
                final IWidget picture = this.getWidget("Layout").getWidget("Picture");
                final double pictureX = picture.getGeometry().getRawPos().getX();
                final double pictureY = picture.getGeometry().getRawPos().getY();
                final double pixelToPercentX = 100.0D / picture.getParent().getSize().getX();
                final double pixelToPercentY = 100.0D / picture.getParent().getSize().getY();

                if (srcwidget.equals(this.getWidget("LayoutX").getWidget("ButtonXAdd"))) {
                    final double newPos = Math.min(pictureX + pixelToPercentX, 100.0);
                    picture.setPos(newPos, pictureY);
                }

                if (srcwidget.equals(this.getWidget("LayoutX").getWidget("ButtonXSub"))) {
                    final double newPos = Math.max(pictureX - pixelToPercentX, 0.0);
                    picture.setPos(newPos, pictureY);
                }

                if (srcwidget.equals(this.getWidget("LayoutY").getWidget("ButtonYAdd"))) {
                    final double newPos = Math.min(pictureY + pixelToPercentY, 100.0);
                    picture.setPos(pictureX, newPos);
                }

                if (srcwidget.equals(this.getWidget("LayoutY").getWidget("ButtonYSub"))) {
                    final double newPos = Math.max(pictureY - pixelToPercentY, 0.0);
                    picture.setPos(pictureX, newPos);
                }
    ...
    public ScreenHUDConfig(final GuiScreen parent) {
        super(parent);
