...
public class ScreenEnchants extends ScreenBase {
    ...
    public ScreenEnchants(GuiScreen parent) {
        ...
        this.getRoot().addWidget("Layout_Title", new LayoutBase(null)).setGeometry(
                new WidgetGeometry(50.0, 10.0, 50.0, 32.0, CType.REL_X, CType.REL_X, WAlign.CENTER, WAlign.TOP));

        this.getRoot().getWidget("Layout_Title").addWidget("ItemStack", new ItemStackDisplay(null)).setGeometry(
                new WidgetGeometry(0.0, 0.0, 32.0, 32.0, CType.ABSXY, CType.ABSXY, WAlign.LEFT, WAlign.TOP));

        this.getRoot().getWidget("Layout_Title").addWidget("LabelName", new LabelFixedFont(null, "None")).setGeometry(
                new WidgetGeometry(40.0, 4.0, 16.0, 16.0, CType.ABSXY, CType.ABSXY, WAlign.LEFT, WAlign.TOP));
        ...
        this.getRoot().addWidget("Table", new ViewTable(null)).setGeometry(
    ...
    public ScreenEnchants setEnchantability(String value) {
        ((LabelFixedFont) this.getRoot().getWidget("Layout_Title").getWidget("LabelEnchantability"))
                .setText(String.format("%s : %s", LangUtil.translateG("enchant.label.enchantability"), value));
        this.getRoot().getWidget("Layout_Title").adjustSize();
        return this;
    }
}
