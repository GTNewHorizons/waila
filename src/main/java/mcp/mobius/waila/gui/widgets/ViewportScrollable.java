...
public class ViewportScrollable extends WidgetBase {
    ...
    public class Escalator extends WidgetBase {
        ...
        public Escalator(final IWidget parent, final int step) {
            this.parent = parent;
            this.step = step;
        }
        ...
        public void draw(final Point pos) {
            ...
            final int offsetScaled = (int) (((double) this.getSize().getY() - (double) sizeCursor + 1) / (double) this.maxValue * (yOffset));
        ...
        public void onMouseClick(final MouseEvent event) {
            if (event.button == 0) {
                final int offsetScaled = this.getTop() + (int) (((double) this.getSize().getY() - (double) sizeCursor + 1) / (double) this.maxValue * (yOffset));
        ...
        public void onMouseDrag(final MouseEvent event) {
            if (this.drag) {
                final int relativeY = (int) event.y - this.getTop();
                final double factor = ((double) this.getSize().getY() - (double) sizeCursor + 1) / (double) this.maxValue;
    ...
    public ViewportScrollable(final IWidget parent) {
        super(parent);
    ...
    public IWidget attachWidget(final IWidget widget) {
    ...
    public IWidget setStep(final int step) {
        this.step = step;
