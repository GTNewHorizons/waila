    ...
    public class Escalator extends WidgetBase {
        ...
        public void draw(Point pos) {
            ...
            UIHelper.drawGradientRect(
                    ...
                    this.getRight(),
                    this.getTop() + offsetScaled + sizeCursor,
                    1,
                    0xffffffff,
                    0xffffffff);
        }
        ...
        public void onMouseClick(MouseEvent event) {
            if (event.button == 0) {
                ...
                if (event.y < offsetScaled) this.yOffset += this.step;
                else if (event.y > offsetScaled + sizeCursor) this.yOffset -= this.step;
                else this.drag = true;
        ...
        public void onMouseRelease(MouseEvent event) {
            if (event.button == 0) this.drag = false;
    ...
    public void onMouseClick(MouseEvent event) {
        if (event.button == 0) {
            if (!this.getWidget("Escalator").isWidgetAtCoordinates(event.x, event.y)) {
                ...
                super.onMouseClick(event);
            } else this.getWidget("Escalator").onMouseClick(event);
