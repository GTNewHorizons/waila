...
public class ViewTable extends WidgetBase {
    ...
    public class Cell extends WidgetBase {
        public Cell(final IWidget parent, final String text, final WAlign align) {
            super(parent);
    ...
    public class Row extends WidgetBase {
        ...
        public Row(final IWidget parent) {
            super(parent);
        ...
        public Row setColumnsWidth(final double... widths) {
            if (this.ncolumns == -1)
        ...
        public Row setColumnsText(final String... strings) {
            if (this.ncolumns == -1)
        ...
        public Row setColumnsAlign(final WAlign... aligns) {
            if (this.ncolumns == -1)
        ...
        public Row setColors(final int bg1, final int bg2) {
            this.bgcolor1 = bg1;
            this.bgcolor2 = bg2;
            ((LayoutBase) this.getWidget("Background")).setBackgroundColors(this.bgcolor1, this.bgcolor2);
            return this;
            ...
            if (!init) {
                ...
                for (int i = 0; i < this.ncolumns; i++) {
                    if (!this.widgets.containsKey(String.format("Cell_%02d", i))) {
                        final Cell cell = (Cell) (this.addWidget(String.format("Cell_%02d", i), new Cell(null, this.texts[i], this.aligns[i])));
    ...
    public ViewTable(final IWidget parent) {
        super(parent);
    ...
    public ViewTable setColumnsWidth(final double... widths) {
        if (this.ncolumns == -1)
            this.ncolumns = widths.length;
        else if (this.ncolumns != widths.length) {
            throw new UIException(String.format("Number of columns mismatch. Expecting %d, got %d", this.ncolumns, widths.length));
    ...
    public ViewTable setColumnsTitle(final String... strings) {
        if (this.ncolumns == -1)
            this.ncolumns = strings.length;
        else if (this.ncolumns != strings.length) {
            throw new UIException(String.format("Number of columns mismatch. Expecting %d, got %d", this.ncolumns, strings.length));
    ...
    public ViewTable setColumnsAlign(final WAlign... aligns) {
        if (this.ncolumns == -1)
            this.ncolumns = aligns.length;
        else if (this.ncolumns != aligns.length) {
            throw new UIException(String.format("Number of columns mismatch. Expecting %d, got %d", this.ncolumns, aligns.length));
    ...
    public ViewTable addRow(final String... strings) {
        final IWidget tableLayout = ((ViewportScrollable) (this.getWidget("Viewport"))).getAttachedWidget();
        tableLayout.setSize(100.0, (this.nrows + 1) * 16);

        final Row newRow = (Row) new Row(null);
