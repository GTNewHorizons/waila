...
public class LayoutCanvas extends LayoutBase {
    ...
    public void handleMouseInput() {
        ...
        switch (type) {
            case CLICK:
                if (targetWidget != null) targetWidget.onMouseClick(event);
                break;
            case DRAG:
                if (targetWidget != null) targetWidget.onMouseDrag(event);
                break;
            case MOVE:
                if (targetWidget != null) targetWidget.onMouseMove(event);
                break;
            case RELEASED:
                if (targetWidget != null) targetWidget.onMouseRelease(event);
                break;
            case WHEEL:
                if (targetWidget != null) targetWidget.onMouseWheel(event);
                break;
            case ENTER:
                if (event.trgwidget != null) event.trgwidget.onMouseEnter(event);
    ...
    public void onWidgetEvent(IWidget srcwidget, Signal signal, Object... params) {
        if (signal == Signal.DRAGGED) this.draggedWidget = srcwidget;
        else super.onWidgetEvent(srcwidget, signal, params);
    }
    ...
    public void onMouseDrag(MouseEvent event) {
        if (this.draggedWidget != null) this.draggedWidget.onMouseDrag(event);
        else super.onMouseDrag(event);
    }
}
