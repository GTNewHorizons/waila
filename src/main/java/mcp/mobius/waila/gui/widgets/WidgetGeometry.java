...
public class WidgetGeometry {
    ...
    public Point getUnalignedPos(IWidget parent) {
        ...
        if (!this.fracPosX && parent != null) x = parent.getPos().getX() + (int) this.x;
        if (!this.fracPosX && parent == null) x = (int) this.x;

        int y = -1;
        if (this.fracPosY)
            y = MathHelper.ceiling_double_int(parent.getPos().getY() + parent.getSize().getY() * this.y / 100D);
        if (!this.fracPosY && parent != null) y = parent.getPos().getY() + (int) this.y;
        if (!this.fracPosY && parent == null) y = (int) this.y;
    ...
    public Point getPos(IWidget parent) {
        ...
        if (this.alignX == WAlign.CENTER) x -= this.getSize(parent).getX() / 2;

        if (this.alignY == WAlign.CENTER) y -= this.getSize(parent).getY() / 2;

        if (this.alignX == WAlign.RIGHT) x -= this.getSize(parent).getX() - 1;

        if (this.alignY == WAlign.BOTTOM) y -= this.getSize(parent).getY() - 1;
    ...
    public Point getSize(IWidget parent) {
        int sx = -1;
        if (this.fracSizeX) sx = MathHelper.ceiling_double_int(parent.getSize().getX() * this.sx / 100D);
        if (!this.fracSizeX) sx = (int) this.sx;

        int sy = -1;
        if (this.fracSizeY) sy = MathHelper.ceiling_double_int(parent.getSize().getY() * this.sy / 100D);
        if (!this.fracSizeY) sy = (int) this.sy;
