...
public class FontHelper {
	...
	*/
    ...
    public static void drawString(final String s, final float x, float y, final TrueTypeFont font, final float scaleX, final float scaleY, final float rotationZ, final float... rgba) {
        ...
        final float tx = (x * sr.getScaleFactor()) + (font.getWidth(s) / 2);
        final float tranx = tx + 2;
        final float trany = y + (font.getLineHeight() / 2);
        ...
        if (s.contains(formatEscape)) {
            final String[] pars = s.split(formatEscape);
            float totalOffset = 0;
    ...
    private static void set2DMode(final FloatBuffer matrixData) {
        final Minecraft mc = Minecraft.getMinecraft();
        final ScaledResolution sr = new ScaledResolution(mc);
